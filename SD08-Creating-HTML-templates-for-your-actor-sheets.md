...
[![boilerplate character sheet annotated](https://mattsmithin.nyc3.digitaloceanspaces.com/assets/boilerplate-character.png)](https://mattsmithin.nyc3.digitaloceanspaces.com/assets/boilerplate-character.png)
...
>
> -   <!-- {% raw %} -->`flexcol`: Included by Foundry itself, this lays out the child elements of whatever element you place this on vertically.
> -   <!-- {% raw %} -->`flexrow`: Included by Foundry itself, this lays out the child elements of whatever element you place this on horizontally.
> -   <!-- {% raw %} -->`flex-center`: When used on something that's using flexrow or flexcol, this will center the items and text.
> -   <!-- {% raw %} -->`flex-between`: When used on something that's using flexrow or flexcol, this will attempt to place space between the items. Similar to "justify" in word processors.
> -   <!-- {% raw %} -->`flex-group-center`: Add a border, padding, and center all items.
> -   <!-- {% raw %} -->`flex-group-left`: Add a border, padding, and left align all items.
> -   <!-- {% raw %} -->`flex-group-right`: Add a border, padding, and right align all items.
> -   <!-- {% raw %} -->`grid`: When combined with the <!-- {% raw %} -->`grid-Ncol`<!-- {% endraw %} --> classes, this will lay out child elements in a grid.
> -   <!-- {% raw %} -->`grid-Ncol`: Replace <!-- {% raw %} -->`N`<!-- {% endraw %} --> with any number from 1-12, such as <!-- {% raw %} -->`grid-3col`<!-- {% endraw %} -->. When combined with <!-- {% raw %} -->`grid`, this will layout child elements in a grid with a number of columns equal to the number specified.
...
In this case, we have a container div with the classes <!-- {% raw %} -->`resources`, <!-- {% raw %} -->`grid`, and <!-- {% raw %} -->`grid-2col`<!-- {% endraw %} -->. The later two classes are the ones driving the layout here; any elements immediately inside the resources div (or <!-- {% raw %} -->`resource`<!-- {% endraw %} --> divs in this case) will be laid out as a two column grid. We're also using the <!-- {% raw %} -->`flex-group-center`<!-- {% endraw %} --> class on the individual <!-- {% raw %} -->`resource`<!-- {% endraw %} --> divs to add some padding, a border, and text centering on them.
...
Something we're doing different for the modifier is that we're using the <!-- {% raw %} -->`numberFormat`<!-- {% endraw %} --> Handlebars helper. This is a special helper that Foundry includes and it allows you to format numbers with <!-- {% raw %} -->`+`<!-- {% endraw %} --> and <!-- {% raw %} -->`-`<!-- {% endraw %} --> signs depending on their value, along with rounding decimals. In this case we're using numberFormat on <!-- {% raw %} -->`ability.mod`, which we derived earlier, and setting the decimals and <!-- {% raw %} -->`+/-`<!-- {% endraw %} --> sign to true.
...
Each tab inside the name if an <!-- {% raw %} -->`<a>`<!-- {% endraw %} --> link with a <!-- {% raw %} -->`data-tab`<!-- {% endraw %} --> attribute. Much like <!-- {% raw %} -->`data-group`, the contents of <!-- {% raw %} -->`data-tab`<!-- {% endraw %} --> don't matter as long as they exactly match a <!-- {% raw %} -->`data-tab`<!-- {% endraw %} --> attribute on your tab content div that gets created later.

## The <!-- {% raw %} -->`<section>`<!-- {% endraw %} --> body

Finally, we also have a <!-- {% raw %} -->`<section>`<!-- {% endraw %} --> element that's used to create the body of sheet where the various pages (which are navigated to via tabs) will live. This doesn't have to be a section element; it could have also been something else like a <!-- {% raw %} -->`<div>`, but sections can be used to tell the browser "this is a discreet chunk of related content", such as multiple paragraphs in a book that are all under the same headline.
...
`{{editor}}`<!-- {% endraw %} --> is a special Handlebars helper that will return a TinyMCE text editor. You'll usually want to format it very similar to this example, substituting the <!-- {% raw %} -->`content`<!-- {% endraw %} --> and <!-- {% raw %} -->`target`<!-- {% endraw %} --> properties with whatever data property you want the editor to make changes to.
...
First, we're defining an ordered listed with the <!-- {% raw %} -->`items-list`<!-- {% endraw %} --> class. We need a header though, so we manually create an <!-- {% raw %} -->`<li>`<!-- {% endraw %} --> tag with the <!-- {% raw %} -->`item`, <!-- {% raw %} -->`flexrow`, and <!-- {% raw %} -->`item-header`<!-- {% endraw %} --> classes. The <!-- {% raw %} -->`item-header`<!-- {% endraw %} --> class is used to let the CSS included with Boilerplate know that this is a header for a list and should use bold font weights.
