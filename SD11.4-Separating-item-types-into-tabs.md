...
After that, scroll down to the owned items tab and change the <!--- {% raw %} --->`{{#each`<!--- {% endraw %} ---> loop from `actor.items` to `actor.gear`, which is the new array we made earlier that includes only the item entities with the `item` sub-type.
...
- The <!--- {% raw %} --->`{{#each`<!--- {% endraw %} ---> loop is on `actor.features` instead of `actor.gear`
...
- To loop through the spell level keys we made in our JS earlier, we use an each loop like <!--- {% raw %} --->`{{#each actor.spells as |spells spellLevel|}}`<!--- {% endraw %} --->. The `spells` identifier will be an array of spells, while the `spellLevel` key will be the numerical spell level.
